...
/**
 * @brief Get the parameter corresponding to a FSK Rx bandwith immediately above
 * the minimum requested one.
 *
 ...
 *
 * \remark To get the actual time-on-air in second, this value has to be divided
 * by the GFSK bitrate in bits per second.
 *
 ...
 *
 * \remark To get the actual time-on-air in second, this value has to be divided
 * by the LoRa bandwidth in Hertz.
 *
 * \param [in] bandwidth
 * \param [in] datarate
 * \param [in] coderate
 ...
 */
static uint32_t
SX1276GetLoRaTimeOnAirNumerator(uint32_t bandwidth, uint32_t datarate,
...
bool SX1276IsChannelFree(uint32_t freq, uint32_t rxBandwidth,
...
uint32_t SX1276GetTimeOnAir(RadioModems_t modem, uint32_t bandwidth,
  ...
  switch (modem) {
  case MODEM_FSK: {
    numerator = 1000U * SX1276GetGfskTimeOnAirNumerator(preambleLen, fixLen,
                                                        payloadLen, crcOn);
    denominator = datarate;
  } break;
  case MODEM_LORA: {
    numerator = 1000U * SX1276GetLoRaTimeOnAirNumerator(
...
static uint32_t SX1276GetGfskTimeOnAirNumerator(uint16_t preambleLen,
  ...
  return (preambleLen << 3) + ((fixLen == false) ? 8 : 0) +
         (syncWordLength << 3) +
         ((payloadLen + (0) + // Address filter size
           ((crcOn == true) ? 2 : 0))
          << 3);
}
...
static void SX1276OnDio2Irq(void *context) {
  ...
  case RF_RX_RUNNING:
    switch (SX1276.Settings.Modem) {
    case MODEM_FSK:
      // Checks if DIO4 is connected. If it is not PreambleDetected is set to
      // true.
